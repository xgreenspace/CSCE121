...
int countNeighborMines(struct minePosition** mineMatrix, int nRows, int nCols, int row, int col)
{
  ...
  // base case: pos has eight adjacent sides
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      if (row + i < 0 || row + i > nRows - 1 || col + j < 0 || col + j > nCols - 1 ) {
        ...
        if (mineMatrix[row + i][col + j].neighborMines == MINE) {
  ...
  // edge case: pos has five adjacent sides

  // corner case: pos has three adjacent sides
